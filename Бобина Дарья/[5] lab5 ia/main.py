...
folder = r'C:\\Users\\dasha\\pythonProject5\\'
folderPlots = 'plots\\'
...
def RewriteCsv():
    ...
    def Gender():
        d1 = pd.Series(map(lambda x: 1 if (
                (x == "M") or (x == "Male") or (x == "m") or (x == "male") or (x == "Mal") or (
                x == "Male ") or (x == "Man") or (x == "Male-ish") or (x == "maile") or (
                        x == "Cis Male") or (x == "Male (CIS)") or (x == "Make") or (x == "msle") or (
                        x == "cis male") or (x == "Cis Male") or (x == "Malr") or (x == "Mail")) else x,
                           df.Gender))
        d2 = pd.Series(map(lambda x: 0 if (
                (x == "female") or (x == "F") or (x == "Woman") or (x == "f") or (x == "woman") or (
                x == "Female") or (x == "Cis Female") or (x == "Femake") or (x == "cis-female/femme") or (
                        x == "Femail (cis)") or (x == "femail")) else 2, d1))
    ...
    def YesNo():
        dfNamesYesNo = ['self_employed','family_history', 'treatment', 'remote_work', 'tech_company', 'benefits',
...
def Statistic():
    df = pd.read_csv(folder+dfName).drop(columns=['Timestamp', 'comments', 'Country', 'state'])
    ...
    for column in ft_names:
        if column == 'Country' or column == 'state' or column == 'Gender':
            continue
...
def Correlation(task):
    ...
    def TreatmentPlt():
        plt.figure(figsize=(12,8) , dpi=100)
        sns.heatmap(corrTreatment, annot=True, vmax=.8, square=True, annot_kws={'size':9}, yticklabels=cols.values, xticklabels=cols.values, color=['red','blue'])
    ...
    def PlotGraphs():
        for i in cols.tolist():
            ...
            ax = dfNew.plot(kind='bar', stacked= True , color=['lime', 'green'], mark_right = True)
            for p in ax.patches:
