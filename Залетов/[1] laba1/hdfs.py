...
class NameNode:
    ...
    def replicate_block(self, block: Block, file_name: FILE_NAME, data: str):
        possible_replication_hosts = self.get_hosts()
        possible_replication_hosts.remove(block.host)

        replica_hosts: list[Host] = list(
            dict(
                sorted(
                    {
                        host: host.get_free_count() for host in possible_replication_hosts
                    }.items(),
        ...
        for replication_host in replica_hosts:
            replication_host_free_blocks = self.get_blocks_free(host=replication_host)
            if not replication_host_free_blocks:
                continue
            replica_block = replication_host_free_blocks[0]
            replica_block.file_name = f"replica_{file_name}_{random.random()}"
            block.replications.append(replica_block)
            replication_host.write_block(replica_block.title, data)
...
class Client:
    ...
    def get_block_data(block: Block, host: Host) -> Optional[str]:
        ...
        for block_replication in block.replications:
            if not block_replication.host.is_alive:
                continue
            return block_replication.host.read_block(block_replication.title)
        return "[блок потерян]"
