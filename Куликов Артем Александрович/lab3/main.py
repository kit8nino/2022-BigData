...
book_years = []
now_on = []
list_now_on = ['0-9', 'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ё', 'Ж', 'З', 'И', 'Й', 'К', 'Л', 'М', 'Н', 'О', 'П', 'Р', 'С',
               'Т', 'У', 'Ф', 'Х', 'Ц', 'Ч', 'Ш', 'Щ', 'Ы', 'Э', 'Ю', 'Я', 'Eng']
i = 0
j = 0
...
for lnk in links:
    ...
    File = open('D:\учеба\lab3\data.csv', 'a')

    with File:
        writer = csv.writer(File, lineterminator="\r")
        for item in range(len(book_names)):
            writer.writerow([str(book_names[item])[2:5]+str(book_names[item])[5:len(str(book_names[item]))-2], str(book_years[item])])
    print("ok")
