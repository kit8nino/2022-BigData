...
class Main(Frame):
    ...
    def start_iu(self):
        ...
        self.txt = tk.Text(
            self.root, font=self._mini_button_font, width=47, height=8, bg="#c1b7c9"
        )
        self.txt.configure(state="disabled")
        ...
        self.entry = tk.Entry(
            self.root, font=self._mini_button_font, width=35, bg="#556066"
        )
...
if __name__ == "__main__":
    nick = f"user_{random.randint(0, 100000)}"
    print(f"{nick=}")
    ...
    socket_thread = threading.Thread(
        target=client.socket_start, args=("192.168.0.106", 8081)
    )
    socket_thread.start()
    game_thread.run()
