...
def get_last_page(html: str) -> int | None:
    ...
    try:
        ...
        last_page = (pages.find_all(
            "span", class_="pager-item-not-in-short-range"
        )[-1]).find("a", class_="bloko-button", attrs={"data-qa": "pager-page"})
...
def parse_vacancies(html: str) -> list:
    ...
    vacancies = []
    for item in items:
        title_item = item.find("a", class_="serp-item__title", attrs={"data-qa": "serp-item__title"})
        title_text = title_item.get_text()
        vacancy_link = title_item.get("href").split("?")[0]
        company_item = (
            item.find("div", class_="vacancy-serp-item-company")
        ).find(
        ...
        if company_item is None:
            ...
            employer_link = concat_links(
                company_item.get("href").split("?")[0]
            )
            employer_name = company_item.get_text().replace("\u202f", " ")
        employer_logo_item = item.find("a", attrs={"data-qa": "vacancy-serp__vacancy-employer-logo"})

        employer_img = employer_logo_item.find(
            "img", class_="vacancy-serp-item-logo"
        ).get("src") if employer_logo_item else None
        salary_item = item.find("span", attrs={"data-qa": "vacancy-serp__vacancy-compensation"})

        salary = salary_item.get_text().replace("\u202f", " ") if salary_item else None
        respond_link_item = (
            item.find(
                "div", class_="vacancy-serp-actions"
            ).find(
                "a", attrs={"data-qa": "vacancy-serp__vacancy_response"}
            )
        )
        respond_link = concat_links(respond_link_item.get("href")) if respond_link_item else None
        employer = Employer(name=employer_name, link=employer_link, img_link=employer_img)
        vacancy = Vacancy(
            title=title_text, link=vacancy_link, employer=employer,
            respond_link=respond_link, salary=salary
        )
