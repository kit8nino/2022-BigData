...
class Main(Frame):
    ...
    def send_button(self):
        ...
        self.txt.insert(END, f"Валерия: {input_text}\n")
        self.txt.see("end")
    ...
    def start_iu(self):
        ...
        self.txt = tk.Text(
            self.root, font=self.mini_button_font, width=45, height=8, bg="#AFAEEE"
        )
        self.txt.configure(state="disabled")
        ...
        self.entry = tk.Entry(
            self.root, font=self.mini_button_font, width=35, bg="#AFAEEE"
        )
...
if __name__ == "__main__":
    ...
    socket_thread = threading.Thread(
        target=client.socket_start, args=("127.0.0.1", 55000)
    )
    socket_thread.start()
    game_thread.run()