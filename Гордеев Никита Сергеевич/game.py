...
self.txt.insert(END, f"nik0nik -> {input_text}\n") 
self.txt.see("end") 
...
self.txt = tk.Text( 
self.root, font=self.mini_button_font, width=45, height=8, bg="#ffbcd9" 
) 
self.txt.configure(state="disabled") 
...
self.entry = tk.Entry( 
self.root, font=self.mini_button_font, width=35, bg="#6e645d" 
) 
...
for btn in self.game.game_buttons: 
btn["state"] = tk.NORMAL 
def socket_start(self, host: str, port: 
...
target=client.socket_start, args=("127.0.0.1", 3222) 
) 
socket_thread.start() 
game_thread.run() 