...
d0 = pd.Series(map(lambda x: np.NaN if ((x < 15)or(x > 100)) else x, df.Age))
print(d0)
...
func = lambda x: 0 if (x=="No") else 1 if (x=="Yes") else np.NaN
d5 = pd.Series(map(func, df.family_history))
d6 = pd.Series(map(func, df.treatment))
d7 = pd.Series(map(func, df.remote_work))
d8 = pd.Series(map(func, df.tech_company))
d9 = pd.Series(map(func, df.benefits))
d10 = pd.Series(map(func, df.care_options))
d11 = pd.Series(map(func, df.wellness_program))
d12 = pd.Series(map(func, df.seek_help))
d13 = pd.Series(map(func, df.anonymity))
d14 = pd.Series(map(func, df.mental_health_consequence))
d15 = pd.Series(map(func, df.phys_health_consequence))
d16 = pd.Series(map(func, df.coworkers))
d17 = pd.Series(map(func, df.supervisor))
d18 = pd.Series(map(func, df.mental_health_interview))
d19 = pd.Series(map(func, df.phys_health_interview))
d20 = pd.Series(map(func, df.mental_vs_physical))
d21 = pd.Series(map(func, df.obs_consequence))
